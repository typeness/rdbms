...
object ManipulationBuilder extends BuilderUtils {
  ...
  def insertRow(query: Insert, relation: Relation): Either[SQLError, Relation] = query match {
    case NamedInsert(_, row) =>
      ...
      } yield appendRow(Row(missing.attributes ::: rowWithCheckedIdentity.attributes), relation, newIdentity)
  ...
  def updateRows(query: Update, relation: Relation): Either[SQLError, Relation] = query.condition match {
    ...
    case Some(condition) =>
      for {
        ...
        updatedRows = matchingRows.map {
          row => row.map(attribute => query.updated.select(attribute.name).getOrElse(attribute))
        }
    ...
    identityOption match {
      ...
      case Some(identity) =>
        if (row.select(identity.name).isDefined) {
          ...
          val primaryKey = BodyAttribute(identity.name, IntegerLiteral(identity.current))
